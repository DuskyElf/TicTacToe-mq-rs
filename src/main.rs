...
async fn main() {
    ...
    loop {
        ...
        interaction(&mut game);
        draw(&game, board_texture, x_texture, o_texture);
        next_frame().await;
    }
}

fn interaction(game: &mut Game) {
...
fn game_logic(game: &mut Game) {
    ...
    for i in 0..3 {
        // Horizontal Checks
        if let Cell::Filled(_) = b[i][0] {
            if b[i][0] == b[i][1] && b[i][1] == b[i][2] {
                exit(0)
            }
        }
        // Vertical Checks
        if let Cell::Filled(_) = b[0][i] {
    ...
    // Diagonal Checks
    if let Cell::Filled(_) = b[0][0] {
        if b[0][0] == b[1][1] && b[1][1] == b[2][2] {
            exit(0)
        }
    }
    if let Cell::Filled(_) = b[0][2] {
